...
export default function ProductSizesSection({
  ...
  return (
    <section
      id="specifications"
      className="border-y border-slate-200 bg-[#f6f7f9] py-16 lg:py-24"
    >
      <div className="mx-auto max-w-[1240px] px-4 lg:px-8">
        ...
        <div className="mt-16 grid items-start gap-8 lg:grid-cols-[240px_1fr_400px] justify-center items-center">
          {/* Size selector */}
          <div
            className={`flex flex-row flex-wrap gap-3 justify-center ${sizes.length > 6 ? 'lg:grid lg:grid-cols-2 lg:w-[280px]' : 'lg:flex-col lg:w-[240px]'}`}
          >
            {sizes.map((size) => (
              <button
                key={size.id}
          ...
          <div className="relative group aspect-square flex flex-col items-center justify-center rounded-[32px] bg-white shadow-sm overflow-hidden lg:ml-8">
          ...
          <div className="rounded-[32px] border border-blue-100 bg-[#dfe7f8] p-5 ">
            <h3 className="text-2xl font-extrabold text-slate-900 lg:text-2xl">
              {activeSize.title}
            </h3>
            ...
            <a
              ...
              onClick={(e) => {
                ...
                onBuyNow?.(activeSize);
                document
                  .getElementById(buyNowHref.replace('#', ''))
